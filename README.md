...
- ~~**`INITCAP` over-capitalizes conjunctions.**~~ **Fixed 2026-08-09 (WMDP2-77).** `SQL/03` now lowercases the conjunction, so the raw export's `Care and Maintenance` survives cleaning intact. The whole chain moved in one commit series: the SQL rule, the regenerated cleaned CSV (40 rows), a forward Netlify migration, the map's colour key, and the Unity constant plus its sample data. See `WA_MINING_PROJECT_PLAN.md` section 1.22.

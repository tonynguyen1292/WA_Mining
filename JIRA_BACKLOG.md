...
- **WMDP2 Sprint 4** — started 2026-07-23 (runs to 16 Aug) with the owner's picked scope: **WMDP2-76** (contract-test the Netlify Functions, created under WMDP2-5) and **WMDP2-20** (custom domain). **Being descoped as of 2026-08-04** — 13 of 25 days elapsed with zero commits against either story; both move out rather than closing 0/2 inside it. Board execution pending: see the 2026-08-04 sync spec below
...
## Board sync spec (2026-08-04) — PENDING, not yet executed

Unlike every log below, this section describes work **not yet done on the board**: the review sitting that produced it had no authenticated Jira session, so nothing was mutated. It is written as an executable spec so the next live session can run it without re-deriving the reasoning. Until then, the board and this file disagree on Sprint 4 — deliberately and visibly, rather than silently.
...
- `feature/unity-shift-supervisor-v2` is **fully merged and 12 commits behind `main`** (`git rev-list --left-right --count origin/main...origin/feature/unity-shift-supervisor-v2` → `12 0`). It holds nothing `main` does not, while the plan, the root README, and the prototype README all still describe it as the live workshop. It must be fast-forwarded from `main` before the next Unity commit: `CLAUDE.md`, the staged-codepoint hook, the `.gitattributes` LF pin, and the `unity-webgl-release` skill all landed after its tip.
- `agent-localhost-to-project-domain-c90c` is fully merged too (0 ahead, 29 behind) — the dead PR #2 branch, still on origin.

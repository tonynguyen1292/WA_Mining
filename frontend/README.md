...
│   │   ├── SiteDetailPage.tsx # single site's full record + related sites in its project
│   │   ├── SiteDetailPage.test.tsx  # related-sites rendering contract (vitest + RTL)
...
- `pages/SiteDetailPage.test.tsx` — the related-sites contract: siblings listed with the viewed site excluded, the view-all link carrying the real `?project=` filter, and a single-site project rendering no section at all (not an empty heading)
...
- The Dashboard's stage/commodity/region chart bars are clickable: `BreakdownChart` takes an optional `linkField`, and a click navigates to `/sites` pre-filtered to that bar's value — the URL-sync work is what makes those destinations shareable links rather than transient state. The link URL is built by `sitesLinkForBreakdown` in `utils/urlFilters.ts`, which returns `null` for the synthetic "Unspecified" bucket (it stands for NULL, which `/sites` can't filter on) so those bars render inert instead of as dead links. The LGA chart deliberately has no `linkField` — the sites list has no LGA filter yet.
- The Dashboard's second row surfaces two things the documentation always described but the UI never showed: the cleaned `lga_name` data (top-10 chart; 65 distinct values would swamp it) and the site-vs-project grain (the "projects with the most sites" panel, fed by `/api/kpis`' `top_projects`, each row linking into the project-filtered Sites view via `?project=<code>`). A provenance strip (source, snapshot date, licence, dataset shape) closes out the page.
- Related sites: `SiteDetailPage` runs a second fetch keyed off the loaded site's `project_code` — the same `?project=` filter the dashboard links use, not a dedicated endpoint — and renders a "Related sites in this project" section only when siblings exist (single-site projects, i.e. most of them, get no empty heading). The `project` filter itself has no FilterBar dropdown (356 projects isn't dropdown material; it arrives via links), so an active one renders as a dismissible "Project: `<code>` ✕" chip — an invisible active filter would make the shortened list look broken.

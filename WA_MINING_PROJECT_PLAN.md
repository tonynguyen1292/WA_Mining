...
#### Found and recorded, not fixed in this pass — **all closed in 1.21**

Logged here so they don't live only in a review transcript (the 1.15 precedent). All documentation-level; none affect running code. Kept as the record of what the review found; the fixes and what each one turned out to be are in 1.21:
...
### 1.21 Documentation drift sweep — ✅ Done (2026-08-04)

**Why:** 1.20's "found and recorded, not fixed" list, closed in the same sitting. None of it affected running code, which is exactly why it was worth doing deliberately rather than never: this repo's documentation *is* part of the deliverable — the README and the decision logs are what a GitHub-first reviewer reads before any source file, and DISCOVERY.md names them as load-bearing for the audience this project is aimed at. Eight items went in; two of them turned out to be more than documentation.

- [x] **The stage-label drift was a data defect, not a prose one.** The docs said "Care and Maintenance" and the app says `Care And Maintenance`, so the first read was "fix the prose". Checking the raw export inverted it: **DMIRS ships `Care and Maintenance` correctly capitalized, and `SQL/03`'s `INITCAP` breaks it.** The cleaning step degrades a value the source already had right, and the degraded form then propagates into the API, the filter dropdowns, the charts, `SQL/05`'s `CASE` literal, the Netlify seed migration, `SiteMarker.cs`, and `InspectionRound.TroubledStage`. Docs corrected to the literal the code actually compares against — with a note at each surface saying the capital "And" is not a typo — and the real fix sequenced as a roadmap item in section 3 rather than smuggled into a docs commit. `data_dictionary.md` deliberately keeps the lowercase form: it documents the *raw* field, where lowercase is correct.
- [x] **`.gitignore` claimed the opposite of the truth.** A `DATABASES/raw/*.csv` rule, commented "too large, downloadable from source... keeps the repo lightweight", sat directly against a tracked raw CSV and `DATABASES/README_database.md`'s "both are tracked deliberately". Harmless for the already-tracked file, but the same README's refresh procedure says "replace `raw/Major_Resource_Projects.csv`" — and a genuinely *new* file dropped there would have been invisible to `git status`. Rule removed, not reworded, with the reasoning in its place.
- [x] `DECISIONS.md`'s Unity-version entry claimed the 6000.5.4f1 migration was "confined to this branch (`main` keeps the 2022.3 project untouched)". False since the I2 merge on 2026-07-24 — `main`'s `ProjectVersion.txt` reads `6000.5.4f1`. Corrected with a dated update rather than a silent rewrite (the house rule for amending an approved document), because *when* it stopped being true is the useful part.
- [x] The prototype README's structure tree predated I1/I2 entirely — no `Scripts/Scenario/`, `Editor/`, `Tests/`, `Screenshots/`, no `DISCOVERY.md` or `FEATURE_INSPECTION_ROUND.md`, no assembly definitions. Rebuilt from the actual file listing and diffed against it. One error caught doing so: the tree first described `RoundTypes.cs` as holding `RoundState`, which is the spec's name for it; the code calls it `RoundPhase`.
- [x] Both READMEs routed readers to `feature/unity-shift-supervisor-v2` for the discovery and spec documents, which have been on `main` since the I2 merge. Corrected to say the artifacts are here and only I3/I4 are on the branch.
- [x] The root README's Repository Structure omitted `netlify/`, `db/`, `.claude/`, `CLAUDE.md`, `netlify.toml`, `drizzle.config.ts`, `.gitattributes`, and the root `package.json`/`tsconfig.json` — the entire production-hosting layer plus the working-agreements layer, invisible in the map a reviewer reads. Added with one-line descriptions, including the "hosting adapter, not the canonical API" framing 1.19 established. The screenshots block was stale too (missing `about.png` and the three owner-captured snapshots).
- [x] The README's test-coverage bullet still listed `/api/kpis` as untested — it gained `test_kpis.py` in 1.13, and section 3 of this file had already recorded the correction, so the README was contradicting the plan. Rewritten against the actual suites (60 backend + 47 frontend), naming what each covers and what genuinely remains uncovered.
- [x] Test counts reconciled: the running total is **60 backend + 47 frontend**, verified by running both. Section 3's roadmap bullet had said 46.
...
- ~~Reconcile `DATABASES/README_database.md` with the committed `DATABASES/raw/` snapshot~~ — **already done in 1.11**, which rewrote that file; this bullet was stale for two weeks and contradicted 1.11's own record. The related `.gitignore` half (a rule that claimed the raw CSV was not tracked) was closed in 1.21.
